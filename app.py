...
def register_recruiter(username, firstname, lastname, email, password, confirmPassword, company, phone_number):
    return {"message": "Recruiter registered successfully", "username": username}
...
def add_job(job_role, company, package, job_description):
    # Implement the logic to add a new job (e.g., store in a database)
    print(f"Job added: {job_role} at {company}, Package: {package}, Description: {job_description}")
...
def get_all_jobs():
    ...
    cursor.execute('SELECT id, job_role, company, package, job_description FROM jobs')
    jobs = cursor.fetchall()

    conn.close()

    job_list = [
        {"id": job[0], "job_role": job[1], "company": job[2], "package": job[3], "job_description": job[4]}
        for job in jobs
    ]
...
def job_page(job_id):
    return render_template('job_details.html')@app.route('/job/<int:job_id>')
def job_page(job_id):
